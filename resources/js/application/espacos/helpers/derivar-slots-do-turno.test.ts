...
/**
 * Rede de segurança da extração: esta função é a ÚNICA fonte de verdade sobre o
 * estado de um slot, consumida tanto pela grade do desktop quanto pela lista do
 * mobile. Se as duas divergissem, um slot poderia aparecer livre no celular e
 * reservado no desktop — e alguém reservaria em cima de outra pessoa.
 */
describe('derivarSlotsDoTurno', () => {
    ...
    /** O primeiro horário do turno manhã é 07:30 - 08:20. */
    ...
    /** Horário sem gestor identificado não pode quebrar a derivação. */
    ...
    /**
     * O caso que mais importa na edição: os horários da própria reserva não
     * podem aparecer como bloqueados por terceiro para o próprio dono.
     */
    ...
    it('cobre todos os horarios do turno para cada dia da semana', () => {
        ...
        // manha tem 6 faixas de horario x 2 dias
        expect(derivados).toHaveLength(12);
        expect(new Set(derivados.map((d) => d.horaLabel)).size).toBe(6);
    });

    /** Turno desconhecido não pode explodir — devolve lista vazia. */
