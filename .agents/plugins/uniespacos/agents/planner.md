...
1. **Ative a Memória (READ_TRIGGER):** Antes de investigar, carregue a skill `memory-management` e consulte o `ai-memory` (via ferramentas MCP) para carregar decisões arquiteturais vigentes e contexto do projeto.
2. **Investigue antes de propor.** Use `Agent(Explore)` para mapear onde o código relevante vive,
