...
# --- Configuration ---
# The root directory of your production application on the server.
APP_DIR="/var/www/uniespacos-production"
# The file to store the image tag of the last successful deployment.
STATE_FILE="$APP_DIR/deployment_state.txt"
# The new image tag passed from the GitHub Actions workflow (e.g., "main").
IMAGE_TAG_NEW="${1}"

# --- Helper Functions ---
function log() {
  echo "✅ [Deploy Production] $1"
}

function log_error() {
  echo "❌ [Deploy Production] $1" >&2
}
