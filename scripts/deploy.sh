...
docker compose -f compose.prod.yml exec -T postgres-production pg_dump -U "${DB_USERNAME}" -d "${DB_DATABASE}" > "storage/backups/backup_$(date +%F_%H-%M-%S).sql"
...
docker compose -f compose.prod.yml exec -T app-production php artisan migrate --force
